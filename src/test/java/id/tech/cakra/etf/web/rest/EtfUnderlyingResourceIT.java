...
public class EtfUnderlyingResourceIT {
    ...
    public void createEtfUnderlying() throws Exception {
        ...
        assertThat(testEtfUnderlying.getUnderlyingId()).isEqualTo(DEFAULT_UNDERLYING_ID);
        assertThat(testEtfUnderlying.getEffectiveDate()).isEqualTo(DEFAULT_EFFECTIVE_DATE);
        assertThat(testEtfUnderlying.getProductId()).isEqualTo(DEFAULT_PRODUCT_ID);
    }
    ...
    public void checkUnderlyingIdIsRequired() throws Exception {
        int databaseSizeBeforeTest = etfUnderlyingRepository.findAll().size();
        // set the field null
        etfUnderlying.setUnderlyingId(null);

        // Create the EtfUnderlying, which fails.
        EtfUnderlyingDTO etfUnderlyingDTO = etfUnderlyingMapper.toDto(etfUnderlying);
        ...
        List<EtfUnderlying> etfUnderlyingList = etfUnderlyingRepository.findAll();
        assertThat(etfUnderlyingList).hasSize(databaseSizeBeforeTest);
    }
    ...
    public void updateEtfUnderlying() throws Exception {
        ...
        EtfUnderlying testEtfUnderlying = etfUnderlyingList.get(etfUnderlyingList.size() - 1);
        assertThat(testEtfUnderlying.getUnderlyingId()).isEqualTo(UPDATED_UNDERLYING_ID);
        assertThat(testEtfUnderlying.getEffectiveDate()).isEqualTo(UPDATED_EFFECTIVE_DATE);
        assertThat(testEtfUnderlying.getProductId()).isEqualTo(UPDATED_PRODUCT_ID);
    }
